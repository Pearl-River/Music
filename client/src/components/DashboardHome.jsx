...
const DashboardHome = () => {
  ...
  return (
    <div className='w-full p-6 flex items-center justify-evenly flex-wrap'>
      <DashboardCard icon={<FaUser className='text-3xl text-textColor' />} name={"Người dùng"} count={allUsers?.length > 0 ? allUsers?.length : 0} />
      <DashboardCard icon={<GiLoveSong className='text-3xl text-textColor'/>} name={"Bài hát"} count={allSongs?.length > 0 ? allSongs?.length : 0}/>
      <DashboardCard icon={<RiUserStarFill className='text-3xl text-textColor'/>} name={"Ca sĩ"} count={allArtists?.length > 0 ? allArtists?.length : 0}/>
