    ...
    <div className='border border-gray-300 rounded-md px-4 py-1 relative cursor-pointer hover:border-gray-400'>
       <p className='text-base tracking-wide text-textColor flex items-center gap-2' 
        ...
        {filterData && filterMenu && (
          ...
          scrollbar-thums-gray-400 py-2 flex flex-col rounded-md shadow-md absolute top-8 left-0'>
            {filterData?.map(data => (
              <div key={data.name} className="flex items-center gap-2 px-4 py-1 hover:bg-gray-200"onClick={() => updateFilterButton(data.name)}>
                {(flag === "Artist" || flag === "Album") && (
                  <img src={data.imageURL} alt="" className='w-8 min-w-[32px] h-8 rounded-full object-cover'/>
                )}
                <p className='w-full'>
